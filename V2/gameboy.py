...
class APU:
    ...
    # ---------- triggers (called from MMU on NRx4 bit7 write) ----------
...
class MMU:
    def __init__(self):
        ...
        self.rom_bank = 1
        self.ram_bank = 0
        # 2 banks of 8KB for CGB (bank switched via VBK); DMG only ever uses bank 0
        self.vram = bytearray(0x4000)
        self.vram_bank = 0
        self.eram = bytearray()
        # 8 banks of 4KB: bank 0 fixed at 0xC000-0xCFFF, switchable bank (1-7) at 0xD000-0xDFFF
        ...
        # MBC3 RTC: rtc_regs are the "live" counting registers, rtc_latch is the
        # frozen snapshot the CPU actually reads (updated by the latch sequence)
        self.rtc_regs = [0] * 5
        self.rtc_latch = [0] * 5
    ...
    def write_byte(self, addr, val):
        ...
        if 0x0000 <= addr < 0x2000:
            ...
            if self.cart_type in [1, 2, 3]:
                ...
                # RTC latch sequence: write 0x00 then 0x01 to snapshot live regs
                if val == 0x00:
            ...
            if offset == 0x00:
                ...
                # VRAM DMA (GDMA/HDMA). We perform the transfer immediately for
                # both general-purpose and HBlank modes - a simplification that
                # skips the real per-HBlank 16-byte pacing, but produces correct
                # end results for the vast majority of games.
                self.io[0x55] = val
...
class CPU:
    ...
    def step(self):
        ...
        if 0x40 <= opcode <= 0x7F:
            ...
            # Unimplemented/invalid opcode: don't kill the whole process over one
            # bad decode. Log it once per opcode and treat it as a NOP so the game
            # can keep running - a crash on a rare or misdecoded opcode is a much
            # worse experience than one frame of a glitch.
...
class PPU:
    def __init__(self, mmu):
        ...
        # (144, 160, 3) frame buffer we write into with vectorized numpy ops,
        # then blit to the surface once per frame instead of calling
        # Surface.set_at() 23,040 times a frame (this was the single biggest
        # performance bottleneck - each set_at is a slow per-pixel Python/C
        # call, ~23k of them per frame tanks fps badly).
        self.frame_buffer = np.zeros((144, 160, 3), dtype=np.uint8)

        # Precomputed lookup: for every possible (byte1<<8)|byte2 tile-row
        # combination, the 8 color indices (0-3) it decodes to, left to right.
        # Turns per-pixel bit twiddling into a single O(1) array lookup.
    ...
    def _cgb_palette_table(self, palette_ram):
        # Vectorized conversion of all 8 palettes x 4 colors -> (32,3) RGB table
    ...
    def render_scanline(self, ly):
        ...
        # bg_priority/bg_zero tracked per-pixel for sprite priority resolution
        ...
        cgb_bg_table = self._cgb_palette_table(self.mmu.bg_palette_ram) if cgb else None
        ...
        if lcdc & 0x02:
            ...
            cgb_obj_table = self._cgb_palette_table(self.mmu.obj_palette_ram) if cgb else None
